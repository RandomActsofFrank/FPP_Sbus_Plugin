...
- **Python 3 and pyserial** – The plugin install script (`fpp_install.sh`) tries to install `python3-serial` (apt) or `pyserial` (pip) if missing. If it’s still not present, install manually: `sudo apt-get install python3-serial`. The daemon will exit with instructions if the module is missing.
